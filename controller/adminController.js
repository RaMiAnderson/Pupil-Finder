...
const getAbsence = (req,res) => {
    const stateConnection = authentificationService.verifyIfAlreadyConnected(req);
    if(stateConnection) res.render('admin/pages/adminAbsence')
    else res.redirect('auth/login');
}

const getAnnonce = (req,res) => {
    const stateConnection = authentificationService.verifyIfAlreadyConnected(req);
    if (stateConnection) res.render("admin/pages/adminAnnonce")
    else res.redirect("auth/login");
}

const getApayer = (req,res) => {
    const stateConnection = authentificationService.verifyIfAlreadyConnected(req);
    if (stateConnection) res.render("admin/pages/adminApayer")
    else res.redirect("auth/login");
}

const getCalendar = (req,res) => {
    const stateConnection = authentificationService.verifyIfAlreadyConnected(req);
    if (stateConnection) res.render("admin/pages/adminCalendar")
    else res.redirect("auth/login");
}

const getEdt = (req,res) => {
    const stateConnection = authentificationService.verifyIfAlreadyConnected(req);
    if (stateConnection) res.render("admin/pages/adminEdt")
    else res.redirect("auth/login");
}

const getGerer = (req,res) => {
    const stateConnection = authentificationService.verifyIfAlreadyConnected(req);
    if (stateConnection) res.render("admin/pages/adminGerer")
    else res.redirect("auth/login");
}

const getNote = (req,res) => {
    const stateConnection = authentificationService.verifyIfAlreadyConnected(req);
    if (stateConnection) res.render("admin/pages/adminNote")
    else res.redirect("auth/login");
}

const getNotif = (req,res) => {
    const stateConnection = authentificationService.verifyIfAlreadyConnected(req);
    if (stateConnection) res.render("admin/pages/adminNotif")
    else res.redirect("auth/login");
}
